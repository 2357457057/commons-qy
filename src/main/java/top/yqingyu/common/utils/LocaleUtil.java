 ...
 * @author YYJ
 * @version 1.0.0
 * @ClassName top.yqingyu.common.utils.LocaleUtil
 * @description
 * @createTime 2023年04月08日 01:53:00
...
public class LocaleUtil {
    ...
    private static boolean isISO3166CountryCode(final String str) {
        return StringUtil.isAllUpperCase(str) && str.length() == 2;
    }
    ...
    private static boolean isISO639LanguageCode(final String str) {
        return StringUtil.isAllLowerCase(str) && (str.length() == 2 || str.length() == 3);
    }
    ...
    private static boolean isNumericAreaCode(final String str) {
        return StringUtil.isNumeric(str) && str.length() == 3;
    }
    ...
    public static List<Locale> localeLookupList(final Locale locale, final Locale defaultLocale) {
        ...
        if (locale != null) {
            ...
            if (!locale.getCountry().isEmpty()) {
                list.add(new Locale(locale.getLanguage(), StringUtil.EMPTY));
            }
    ...
    public static Locale toLocale(final String str) {
        ...
        if (str.isEmpty()) { // LANG-941 - JDK 8 introduced an empty locale where all fields are blank
            return new Locale(StringUtil.EMPTY, StringUtil.EMPTY);
        }
        ...
        if (ch0 == '_') {
            ...
            if (len == 3) {
                return new Locale(StringUtil.EMPTY, str.substring(1, 3));
            }
            if (len < 5) {
                throw new IllegalArgumentException("Invalid locale format: " + str);
            }
            ...
            return new Locale(StringUtil.EMPTY, str.substring(1, 3), str.substring(4));
        }
